...
let totalCommits = 0;
githubData.forEach ((commitEvent) => {
    if(commitEvent.payload.hasOwnProperty("commits")){
        totalCommits += commitEvent.payload.commits.length
    }
});
console.log("Challenge 1: total commits = ", totalCommits)
...
let eventTypeArray = [];
githubData.forEach((propertyType) => {
    eventTypeArray.push(propertyType.type)
}
);

let counts = {}
eventTypeArray.forEach((number) => {
    counts[number] = (counts[number] || 0) + 1;
})
console.log("Challenge 2: event types = ", counts)
...
githubData.forEach((myArray) => {
    if(myArray.type === "PullRequestEvent" && myArray.payload.pull_request.merged === true) {
        newArray.push(myArray.payload.pull_request.user.login)
        // console.log("myarray: ", myArray.payload.pull_request.user.login)
    }
})
console.log("Challenge 3: Github User Pull Requests Approved by Steve: ", newArray)
...
let eventRepos = [];
githubData.forEach((repoName) => {
    eventRepos.push(repoName.repo.name)
});

let repoCounts = {};
eventRepos.forEach((number) => {
    repoCounts[number] = (repoCounts[number] || 0) + 1;
});
console.log("Challenge 4: Repo types = ", repoCounts)
...
let keys = [];
let values = [];
githubData.forEach((hasCommits) => {
    if (hasCommits.payload.hasOwnProperty("commits")){
        keys.push(hasCommits.id)
        values.push(hasCommits.payload.commits.length);
    }
});
let result = [];
keys.forEach((key, i) => result[key] = values[i]);
console.log("Challenge 5: Event with Most Commits = ", result )
...
let languagesArr = [];
githubData.forEach((progLanguage) => {
    debugger;
    if(progLanguage.payload.hasOwnProperty("pull_request")){
        languagesArr.push(progLanguage.payload.pull_request.head.repo.language) 
        languagesArr.push(progLanguage.payload.pull_request.base.repo.language) 
    }
});
// the following code removes duplicates
let unique = [...new Set(languagesArr)];
console.log("Challenge 6: Languages Affected (duplicates removed)= ", unique)
...
let langCounts = []
languagesArr.forEach((number) => {
    langCounts[number] = (langCounts[number] || 0) + 1;
})


console.log("Challenge 7: total language = ", langCounts)